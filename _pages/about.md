...
when only $\ell\ll N$ nodes have been sampled, and $\mathcal{K}_k$ nodes with connectivity $k$ have been visited (this includes repeated visits to the same node.) This formula, generated by the this formalism, looks very odd when compared to what is typically done when nodes are sampled uniformly,
